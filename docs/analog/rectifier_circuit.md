...
由于我们的现在一个半周要过两个管子,所以输出的电压其实是$v_{s}-2v_{d}$.但是我们注意到,此时的截止管只要负责 hold 住自己旁边的管子了,此时 PIV 是:
...
虽然我们上面介绍了很多种整流器,但是不得不承认,这些整流器都只能将我们的交流电变成所谓的**脉动直流电**,这种直流电还是不能直接用于我们的电路元件,因为它们说到底都还不是我们想要的直流电.  
我们的一个解决方案是在我们的负载上并联一个电容,一般称为**滤波电容**.  

![f_cap](images/filtering_cap.png)

在此时,假设我们的二极管是理想二极管,输入是一个正弦波.  
当一开始我们将$v_{I}$一直提高,此时我们的电容器电压也不断提高,因而我们可以一直对电容充电,直到$v_{c}=v_{p}$.此时我们的电容器的电压(即二极管阴极电压)恒大于/等于我们二极管的阳极电压.此时二极管直接视为开路,在不接负载的条件下,电容将一直保持电压$v_{p}$.图像如下所示:

![waveform](images/filter_waveform.png)

这看起来很适合作为直流电源,但是没有接负载,我们不妨考虑接上电阻为$R$的负载的情况,如下图所示.
![pic](images/Cap_recmitR.png)
这个时候每一个周期内存下来的峰值电压$V_{P}$就会被外电阻分掉一点,所以波形就长成充满,掉一点,再充满
的样子,$v-t,i-t$如下图所示:
