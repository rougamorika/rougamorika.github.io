...
我们有后面作为等比级数:
$$
\underbrace{\frac{1}{3^s} + \frac{1}{4^s}}_{2}; \underbrace{\frac{1}{5^s} + \frac{1}{6^s} + \frac{1}{7^s} + \frac{1}{8^s}}_{2^2}; \underbrace{\frac{1}{9^s} + \cdots + \frac{1}{16^s}}_{2^3}; \cdots; \underbrace{\frac{1}{(2^{k-1} + 1)^s} + \cdots + \frac{1}{(2^k)^s}}_{2^{k-1}}; \cdots
$$
由于$s=1+\sigma$,所以他们一定小于对应的$\sigma$几何级数:
$$
\underbrace{\frac{1}{3^\sigma} + \frac{1}{4^\sigma}}_{2}; \underbrace{\frac{1}{5^\sigma} + \frac{1}{6^\sigma} + \frac{1}{7^\sigma} + \frac{1}{8^\sigma}}_{2^2}; \underbrace{\frac{1}{9^\sigma} + \cdots + \frac{1}{16^\sigma}}_{2^3}; \cdots; \underbrace{\frac{1}{(2^{k-1} + 1)^\sigma} + \cdots + \frac{1}{(2^k)^\sigma}}_{2^{k-1}}; \cdots
$$
...
首先,我们注意到$n$一定是比$\log(\log n)$更高阶的无穷大,所以我们考虑其与$n^{2}$的关系,注意到对于极限
$$
\lim_{n\to \infty}\frac{n^{2}}{n^{\log\log n}}
$$
注意到上面的极限一定为0,如果你注意不到,我们可以这么玩一下:
$$
...
我们发现换底带来不必要的麻烦(换成10那么级数的式子反而变得更复杂),交换真数和底数不改变式子的结构,相当于什么都没做,所以只能先尝试写成$e^{\log a}$的结构,即
$$
